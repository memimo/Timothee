function [center, nCouldHaveFired, nFired ] = competitiveHebbianCenterLearning(nMap,nIter,getMap,weightSharingMode,center,patch,recFilter,PARAM)
...
%     map = getMap(imOrder(currentImIdx));
    ...
    for iCenter=1:nActiveCenter
        if weightSharingMode
            ...
            im = map{1};
            normIm = norm(im(:));
            center(iCenter).minDistance = 1-dot(im(:),center(iCenter).patch(:))/normIm;
    ...
    %     nCouldHaveFired(iter) = sum(activity >= [center.activityThreshold]);
    nCouldHaveFired(iter) = sum(activity >= 1);

        i = 1;
    %     disp(activity)
    nFiredThisTime = 0;
    %     for i=1:PARAM.n
    while activity(i) >= 1% firing criterion
        %         if activity(i) >= center(i).activityThreshold
        %         disp([dist ;idx])
        ...
        %         % debug
        %         d1 = 1 - dot(bestPatch(:),center(idx(i)).patch(:)) / norm(bestPatch(:));
...
% Reporting
% compute delta theta
center.evolDeltaTheta(iter) = acos(min(1,dot(center.patch(:),newCenterPatch(:))));
% compute Theta
center.evolTheta(iter) = acos(min(1,dot(center.patch(:),normedBestPatch(:))));
% dist
center.evolDist(iter) = center.maxActivity;
% band
center.evolBand(iter) = center.minBand;
% dist
center.evolThetaOriginal(iter) = acos(min(1,dot(newCenterPatch(:),center.originalPatch(:))));
...
for iCenter=1:n
    ...
    center = 1+.0*rand([RFSize nFilter]);
